...
class TikTokScraper:
    ...
    async def get_user_profile(self, username: str) -> UserProfile:
        ...
        try:
            ...
            # Detect bot detection errors and raise immediately for fast fallback
            if any(keyword in error_msg for keyword in ['empty response', 'detecting', 'bot', 'blocked']):
                logger.error(f"Bot detection error for {username}: {e}")
    ...
    async def scrape_user_videos(
        ...
        try:
            ...
            # Detect bot detection errors and raise immediately for fast fallback
            if any(keyword in error_msg for keyword in ['empty response', 'detecting', 'bot', 'blocked']):
                logger.error(f"Bot detection error while scraping {username}: {e}")
                raise RuntimeError(f"TikTok bot detection: {e}")
